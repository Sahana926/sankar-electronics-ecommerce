...
/**
 * PowerTools Component
 * 
 * Dedicated page for power tools with sidebar filtering by subcategory
 */
function PowerTools() {
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'power tools'
    if (sub.includes('drill') || title.includes('drill')) keyword = 'drills, power tools'
    else if (sub.includes('saw') || title.includes('saw')) keyword = 'saws, power tools'
    else if (sub.includes('grinder') || title.includes('grinder')) keyword = 'grinders, tools'
    else if (sub.includes('sand') || title.includes('sand')) keyword = 'sanders, tools'
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Power Tools')
        ...
        const determineSubcategory = (product) => {
          ...
          if (allText.includes('sand')) return 'sanders'
          if (allText.includes('grind')) return 'grinders'
          if (allText.includes('saw')) return 'saws'
          if (allText.includes('drill')) return 'drills'
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/power-tools' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
        ...
        <div className="page-header">
          <h1>Power Tools</h1>
          <p className="subtitle">Drills, saws, grinders, and sanders for professional work</p>
        </div>
