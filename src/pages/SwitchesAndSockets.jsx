...
/**
 * Switches & Sockets Component
 * 
 * Dedicated page for switches and sockets with sidebar filtering by subcategory
 */
function SwitchesAndSockets() {
  ...
  const fallbackImageFor = (item) => {
    ...
    if (sub.includes('socket') || title.includes('socket')) keyword = 'power outlet, electrical socket'
    else if (sub.includes('switch') || title.includes('switch')) keyword = 'light switch, electrical switch'
    else if (sub.includes('light') || title.includes('light')) keyword = 'led light bulb, lighting'
    else if (sub.includes('fan')) keyword = 'ceiling fan, table fan'
    else if (sub.includes('wire') || sub.includes('cable')) keyword = 'electrical cable, wires'
    else if (sub.includes('mcb') || sub.includes('distribution')) keyword = 'circuit breaker, mcb'
    else if (sub.includes('access')) keyword = 'electrical accessories'
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Switches & Sockets')
        ...
        const determineSubcategory = (product) => {
          ...
          // Check features for wattage/amperage
          ...
          if ((allText.includes('socket') || allText.includes('sock-')) && !allText.includes('switch')) {
            return 'socket'
          } else if (allText.includes('fan regulator') || allText.includes('regulator')) {
            return 'fan-regulator'
          } else if (allText.includes('plate') || allText.includes('switchboard') || allText.includes('combined') || allText.includes('combination')) {
            return 'switchboard'
          } else if (allText.includes('16a') || allText.includes('16 a') || allText.includes('16amp')) {
            return 'modular-16a'
          } else if (allText.includes('6a') || allText.includes('6 a') || allText.includes('6ax') || allText.includes('6amp')) {
            return 'modular-6a'
          } else if (allText.includes('switch')) {
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/switches-sockets' } })
      return
    }

    // Determine variant key and price if variants exist (supports backend colorVariants)
    ...
    <main className="main-content">
      <div className="container">
            ...
            {Object.entries(categoryData).map(([key, data]) => (
              ...
              >
                <div 
                  ...
                  onClick={() => {
                    if (key === 'electronics') {
                      // Stay on current page or navigate to shop
                      navigate('/shop')
                    } else {
                      navigate('/shop')
                    }
        ...
        <div className="page-header">
          <h1>Switches & Sockets</h1>
          <p className="subtitle">Premium quality electrical switches and sockets</p>
        </div>
          ...
          <section className="products-section">
            ...
            <div className="products-grid">
              {filteredProducts.map(item => {
                // Support both old variants structure and new colorVariants from backend
                const variants = Array.isArray(item.colorVariants) && item.colorVariants.length > 0
                  ? item.colorVariants.map(cv => ({
                      id: cv.colorName || cv.id,
                      label: cv.colorName || cv.label,
