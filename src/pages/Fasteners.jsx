 ...
 * Fasteners Component
 * 
 * Dedicated page for fasteners with sidebar filtering by subcategory
 */
function Fasteners() {
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'hardware fasteners'
    if (sub.includes('screw') || title.includes('screw')) keyword = 'screws, hardware'
    else if (sub.includes('bolt') || title.includes('bolt')) keyword = 'bolts, hardware'
    else if (sub.includes('nut') || title.includes('nut')) keyword = 'nuts, hardware'
    else if (sub.includes('washer') || title.includes('washer')) keyword = 'washers, hardware'
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Fasteners')
        ...
        const determineSubcategory = (product) => {
          ...
          if (allText.includes('washer')) return 'washers'
          if (allText.includes('nut')) return 'nuts'
          if (allText.includes('bolt')) return 'bolts'
          if (allText.includes('screw')) return 'screws'
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/fasteners' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
        ...
        <div className="page-header">
          <h1>Fasteners</h1>
          <p className="subtitle">Screws, bolts, nuts, and washers for every build</p>
        </div>
