...
/**
 * PlumbingHardware Component
 * 
 * Dedicated page for plumbing hardware with sidebar filtering by subcategory
 */
function PlumbingHardware() {
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'plumbing hardware'
    if (sub.includes('pipe') || title.includes('pipe')) keyword = 'pipes, plumbing'
    else if (sub.includes('faucet') || title.includes('faucet')) keyword = 'faucets, plumbing'
    else if (sub.includes('valve') || title.includes('valve')) keyword = 'valves, plumbing'
    else if (sub.includes('fitting') || title.includes('fitting')) keyword = 'fittings, plumbing'
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Plumbing Hardware')
        ...
        const determineSubcategory = (product) => {
          ...
          if (allText.includes('fitting')) return 'fittings'
          if (allText.includes('valve')) return 'valves'
          if (allText.includes('faucet')) return 'faucets'
          if (allText.includes('pipe')) return 'pipes'
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/plumbing-hardware' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            ...
                            } else if (label === 'Plumbing Hardware') {
                              navigate('/plumbing-hardware')
                            } else {
                              navigate('/shop')
                            }
        ...
        <div className="page-header">
          <h1>Plumbing Hardware</h1>
          <p className="subtitle">Pipes, faucets, valves, and fittings for all plumbing needs</p>
        </div>
