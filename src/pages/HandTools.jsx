 ...
 * HandTools Component
 * 
 * Dedicated page for hand tools with sidebar filtering by subcategory
 */
function HandTools() {
  ...
  const subcategories = [
    ...
    { id: 'screwdrivers', label: 'Screwdrivers' },
    { id: 'pliers', label: 'Pliers' }
  ]
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'hand tools'
    if (sub.includes('wrench') || title.includes('wrench')) keyword = 'wrenches, tools'
    else if (sub.includes('hammer') || title.includes('hammer')) keyword = 'hammers, tools'
    else if (sub.includes('screwdriver') || title.includes('screwdriver')) keyword = 'screwdrivers, tools'
    else if (sub.includes('pliers') || title.includes('pliers')) keyword = 'pliers, tools'
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Hand Tools')
        ...
        const determineSubcategory = (product) => {
          // Use the saved subcategory from database if it exists
          if (product.subcategory) {
            return product.subcategory
          }
          
          // Fallback: determine from product name/features
          ...
          if (allText.includes('pliers')) return 'pliers'
          if (allText.includes('screwdriver')) return 'screwdrivers'
          if (allText.includes('hammer')) return 'hammers'
          if (allText.includes('wrench')) return 'wrenches'
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/hand-tools' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
        ...
        <div className="page-header">
          <h1>Hand Tools</h1>
          <p className="subtitle">Wrenches, hammers, screwdrivers, and pliers for every job</p>
        </div>
