 ...
 * Lighting Component
 * 
 * Dedicated page for lighting products with sidebar filtering by subcategory
 */
function Lighting() {
  ...
  const subcategories = [
    { id: 'tube-light', label: 'Tube Light' },
    { id: 'ceiling-light', label: 'Ceiling Light' },
    { id: 'bulb', label: 'Bulb' },
    { id: 'decorative-light', label: 'Decorative Light' }
  ]
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'led lighting product'
    if (sub.includes('tube') || title.includes('tube') || title.includes('batten')) keyword = 'led tube light'
    else if (sub.includes('ceiling') || title.includes('ceiling') || title.includes('panel') || title.includes('downlight')) keyword = 'ceiling light'
    else if (sub.includes('decorative') || title.includes('decorative') || title.includes('chandelier') || title.includes('pendant') || title.includes('string')) keyword = 'decorative light'
    else if (sub.includes('bulb') || title.includes('bulb')) keyword = 'led bulb'
  ...
  const sampleProducts = useMemo(() => [
      ...
      },
    {
      ...
      description: 'Bright LED bulb for larger spaces',
      price: 170,
      subcategory: 'bulb',
      stock: 140,
      },
      ...
      },
    {
      ...
      description: 'Bright LED tube light',
      price: 320,
      subcategory: 'tube-light',
      stock: 90,
      },
    {
      id: 'light-5',
      icon: '⬜',
      title: 'LED Ceiling Panel Light 18W Square',
      description: 'Slim square ceiling panel light',
      price: 450,
      ...
      },
    {
      id: 'light-6',
      icon: '⬜',
      title: 'LED Downlight 12W Round',
      description: 'Recessed ceiling light with uniform illumination',
      price: 520,
      subcategory: 'ceiling-light',
      stock: 70,
      },
    {
      ...
      price: 1350,
      subcategory: 'decorative-light',
      stock: 45,
      },
    {
      id: 'light-8',
      icon: '✨',
      title: 'String Decorative Lights',
  ...
  useEffect(() => {
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Lighting')
        const res = await fetch(`${API_BASE}/api/products?category=${category}&limit=200`)
        const data = await res.json()
        if (!res.ok) throw new Error(data.message || 'Failed to load products')
        const mapped = (data.data || []).map(p => {
          const providedSubcategory = (p.subcategory || '').toLowerCase()
          // Auto-detect subcategory from product name when not provided
          const name = (p.name || '').toLowerCase()
          let subcategory = providedSubcategory
          
          if (!subcategory) {
            if (name.includes('tube light') || name.includes('batten') || name.includes('tubelight')) {
              subcategory = 'tube-light'
            } else if (name.includes('bulb')) {
              subcategory = 'bulb'
            } else if (name.includes('ceiling') || name.includes('panel') || name.includes('downlight')) {
              subcategory = 'ceiling-light'
            } else if (name.includes('decor') || name.includes('chandelier') || name.includes('pendant') || name.includes('string')) {
              subcategory = 'decorative-light'
            }
          }
          ...
          return {
            ...
            stock: p.stockQty ?? 0,
            imageUrl: Array.isArray(p.images) && p.images.length > 0 ? p.images[0] : fallbackImageFor({ id: p._id, title: p.name, subcategory: subcategory }),
            productRaw: p,
          }
        })
  ...
  const handleAddToCart = async (e, item) => {
    ...
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/lighting' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
        ...
        <div className="page-header">
          ...
          <p className="subtitle">Energy-efficient LED lighting for every need</p>
        </div>
