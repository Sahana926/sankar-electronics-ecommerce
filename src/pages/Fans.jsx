...
// Fans page styled to match Lighting layout
function Fans() {
  ...
  const subcategories = [
    ...
    { id: 'table-fan', label: 'Table Fan' },
    { id: 'pedestal-fan', label: 'Pedestal Fan' },
  ...
  const fallbackImageFor = (item) => {
    ...
    let keyword = 'ceiling fan'
    if (sub.includes('table') || title.includes('table')) keyword = 'table fan'
    else if (sub.includes('pedestal') || title.includes('pedestal')) keyword = 'pedestal fan'
    else if (sub.includes('exhaust') || title.includes('exhaust')) keyword = 'exhaust fan'
  ...
  useEffect(() => {
    const loadCategory = async () => {
      try {
        const category = encodeURIComponent('Fans')
        ...
        const determineSubcategory = (product) => {
          ...
          if (allText.includes('ceiling')) return 'ceiling-fan'
          if (allText.includes('table')) return 'table-fan'
          if (allText.includes('pedestal') || allText.includes('stand')) return 'pedestal-fan'
          if (allText.includes('exhaust') || allText.includes('ventilat')) return 'exhaust-fan'
          
          return 'ceiling-fan' // default to ceiling fan
        }
  ...
  const handleAddToCart = async (e, item) => {
    e.stopPropagation()
    if (!isAuthenticated) {
      navigate('/login', { state: { from: '/fans' } })
      return
    }
    ...
    <main className="main-content">
      <div className="container">
        ...
        <div className="page-header">
          <h1>Fans</h1>
          <p className="subtitle">Ceiling, table, pedestal, and exhaust fans</p>
        </div>
